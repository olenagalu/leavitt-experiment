"""
LEAVITT SERVER - Multi-trial, CIRCLE topology, variable Jetson count (2-5)

Runs the experiment server, coordinating Jetson agents
by assigning private figure sets, routing turn-based messages to circle neighbors, and
...
def get_jetson_number(name):
    match = re.search(r"jetson\s*(\d+)", str(name).strip().lower())
    if not match:
        return None
...
class LeavittServer:
    ...
    def _build_agent_prompt(self, agent_id, agent_name, symbols, num_agents):
        left_neighbor, right_neighbor = get_circle_neighbors(agent_id, num_agents)
        symbols_str = ", ".join(symbols)
        history = self.agent_histories.get(agent_id, [])[-5:]

        if history:
            history_text = ""
            for msg in history:
                history_text += f"[{msg.get('sender', 'SYSTEM')}]: {msg.get('text', '')}\n"
        else:
            history_text = "(No visible messages yet.)\n"

        return f"""
Communication topology: CIRCLE

You are Agent {agent_id} out of {num_agents} agents.

You can communicate only with your two neighbors:
- Left neighbor: Agent {left_neighbor}
- Right neighbor: Agent {right_neighbor}

When you send a CHAT message, only these two neighbors will receive it.
Agents who are not your neighbors will NOT see your message directly.

Do not assume every agent has seen the same messages.
Other agents may know different information.
If you learn useful information, explain it clearly so your neighbors can pass it around the circle.

Your goal is still to help the group find the one figure shared by all agents.

Use your private figure list and only the recent messages visible to you.

PRIVATE FIGURES FOR {agent_name}:
{symbols_str}
...
"""
    ...
    def handle_new_client(self, client_socket, client_address):
        try:
            ...
            with self.lock:
                hostname = str(resp.get("hostname", "")).strip().lower().split(".", 1)[0]
                if not hostname:
        ...
        while self.running:
            with self.lock:
                ...
                for sock in list(self.turn_order):
                    ...
                    if not info:
                        continue
                    jetson_number = get_jetson_number(info.get("hostname", ""))
                    if jetson_number is not None:
                        connected.append((jetson_number, sock))

                if len(connected) >= needed:
                    selected = sorted(connected, key=lambda x: x[0])[:needed]
    ...
    def run_experiment(self):
        ...
        print("  Communication topology: CIRCLE")
        ...
        while self.running and not trial_finished:
            ...
            for current_sock in active_socks:
                ...
                self._send(current_sock, {"type": "your_turn", "prompt": prompt})
                ...
                if resp.get("type") == "chat":
                    ...
                    if text:
                        ...
                        neighbor_ids = get_circle_neighbors(speaker_id, self.num_agents)
                        visible_agent_ids = list(dict.fromkeys([speaker_id] + neighbor_ids))
                        self.conversation_log.append({
                            "round": round_number,
                            "turn": turn_count + 1,
                            "sender": speaker,
                            "routed_to": visible_agent_ids,
                            "text": text,
                            "elapsed": round(time.time() - self.start_time, 2),
                        })
                        # Circle topology: save and send chat only to sender and two neighbors.
                        for agent_id in visible_agent_ids:
                            self.agent_histories.setdefault(agent_id, []).append({
                                "sender": speaker,
                                "text": text,
                            })
                        self._send_to_agent_ids(
                            {"type": "chat", "sender": speaker, "text": text},
                            visible_agent_ids,
                            agent_socks,
                        )
                        print(f"{speaker} CHAT routed to neighbors {neighbor_ids}: {text}")
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("Usage: python circleS.py <host> <port>")
        print("Example: python circleS.py 0.0.0.0 5001")
        sys.exit(1)
