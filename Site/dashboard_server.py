...
CIRCLE_ANSWER_ALLOWED_FROM_ROUND_BY_AGENTS = {2: 2, 3: 3, 4: 4, 5: 5}
HOSTNAME_TO_AGENT = {
    "jetson1": "Agent1",
    "jetson2": "Agent2",
    "jetson3": "Agent3",
...
class DashboardExperiment:
    ...
    def start_trial(self, topology, num_agents):
        with self.lock:
            if self.trial_active or self.trial_requested:
                return False, "A trial is already running or waiting for Jetsons."
            self.topology = topology if topology in ("broadcast", "circle") else "broadcast"
        ...
        self.add_event("trial_requested", {"topology": self.topology, "num_agents": self.num_agents})
    ...
    def send_turn_request(self, sock, speaker_id, speaker, round_number, can_answer):
        if self.topology == "circle":
            ...
            self.send_json(sock, {"type": "your_turn", "round": round_number, "can_answer": can_answer})
    ...
    def run_experiment(self):
        ...
        while self.running and not trial_finished:
            ...
            for current_sock in list(active_socks):
                ...
                can_answer = round_number >= answer_allowed_from_round
                ...
                resp = self.recv_json(current_sock, timeout=300)
