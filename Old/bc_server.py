...
def normalize_candidate_message(text):
    """
    Converts candidate-list messages into a stable sorted tuple.
    Returns None if the message is not a candidate-list message.
    """
    text_lower = text.lower()

    if "updated candidate list" not in text_lower:
        return None

    valid_figures = set(FIGURES)
...
class LeavittServer:
    ...
    def run_experiment(self):
        ...
        stagnation_limit = self.num_agents * 3
            ...
            for current_sock in active_socks:
                ...
                if resp.get("type") == "chat":
                    ...
                    if text:
                        ...
                        # Stagnation detection: same candidate-list state repeated ~3 rounds.
                        candidate_state = normalize_candidate_message(text)
                        if candidate_state is not None:
                            ...
                            if same_candidate_count >= stagnation_limit:
                                stalled = True
                                trial_finished = True
                                stall_reason = "same candidate list repeated for 3 rounds"
                                print("\n[STALLED] Same candidate list repeated for 3 rounds.")
                                print(f"[STALLED] Repeated candidate state: {candidate_state}")
                                break
        ...
        stop_reason = "trial stopped by user" if self.stop_current_trial else None

        result = {
            "type": "experiment_end",
            "result": "stopped" if self.stop_current_trial else ("stalled" if stalled else ("correct" if found else ("incorrect" if self.answers else "incomplete"))),
            "reason": stop_reason if self.stop_current_trial else stall_reason,
    ...
    def start(self):
        try:
            ...
            print("  Turn mode: round-based, unlimited rounds until correct answer")
            print(f"{'=' * 55}\n")
