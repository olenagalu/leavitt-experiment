...
const DEMO_MODEL_NAME = "gemma3:4b";
...
function updateActiveModel(snapshot) {
  if (state.source === "demo") {
    activeModel.textContent = `${DEMO_MODEL_NAME} (simulated)`;
...
function updateCopy() {
  ...
  } else if (state.liveConnected || snapshot) {
  ...
  renderResults(state.source === "demo" ? state.demoResults : (snapshot?.results || []));
}
...
function demoStep() {
  ...
  if (state.messageCount % state.agentCount === 0) {
    state.demoResults.push({
      topology: state.mode,
      success: true,
      submitted_answer: "circle",
      correct_answer: "circle",
      temperature: state.ollamaOptions.temperature,
      total_messages: state.messageCount,
      time_seconds: Math.max(1, Math.round((Date.now() - state.demoTrialStartedAt) / 1000)),
    });
    state.demoTrialStartedAt = Date.now();
  }
...
function renderResults(results) {
  ...
  if (!results.length) {
    const emptyLabel = state.source === "demo" ? "The first simulated result will appear after five messages." : "No live trials yet.";
    resultsBody.innerHTML = `${renderResultTopologyRow(activeTopology)}<tr><td colspan="7">${emptyLabel}</td></tr>`;
    return;
  }
